...
import { useTopDifficulties } from '../lib/hooks/useTopDifficulties';
import { TopUserDifficulty } from '../lib/types/dashboard';
import { formatHashrate, formatNumber } from '../utils/helpers';

interface TopUserDifficultiesClientProps {
  initialData: { data: TopUserDifficulty[]; generatedAt: string };
  limit: number;
}

const SMALL_LIMIT = 10;

export default function TopUserDifficultiesClient({
  initialData,
  limit,
}: TopUserDifficultiesClientProps) {
  const { data, isLoading, error, refetch } = useTopDifficulties(
    limit,
    initialData
  );
