...
const wouldCreate = lines.filter(l => l.includes('DRY RUN: WOULD CREATE') || l.includes('WOULD CREATE'));

if (wouldCreate.length === 0) {
  console.log('checkAndMaybeFixPnpmLinks: no missing top-level links detected.');
  process.exit(0);
}

console.log(`checkAndMaybeFixPnpmLinks: detected approx ${wouldCreate.length} missing top-level links.`);
...
if (process.env.FIX_PNPM_LINKS === '1') {
  ...
  const r = spawnSync(process.execPath, [fixer], { stdio: 'inherit', encoding: 'utf8' });
...
ask(prompt, (answer) => {
  ...
  if (normalized === 'y' || normalized === 'yes') {
    ...
    const res = spawnSync(process.execPath, [fixer], { stdio: 'inherit', encoding: 'utf8' });
    if (res.error) {
      console.error('fixPnpmLinks failed; please run `pnpm run fix-pnpm-links` manually and inspect errors.');
      process.exitCode = 2;
    } else {
